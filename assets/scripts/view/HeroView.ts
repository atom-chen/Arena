    ...
    setHero(h: Hero) {
        ...
        h.onAttack.add(this, info => {
            ...
            if (otherView) {
                ...
                //TODO: depending on hero attack time
                


                // if (this._hero.classType == HeroClassType.Mellee && !info.u) {
                //     cc.tween(this.node)
                //         .by(this._hero.attackDelayMs / 2500, { position: cc.v2(100, 0) })
                //         .by(this._hero.attackDelayMs / 2500, { position: cc.v2(-100, 0) })
                //         .start()
                // }
            } else {
    ...
    private _getParticleAnim(enemy: cc.Node) {
        if (this._hero.type == HeroType.Archer) {
            ...
            let moveTweens = Array.from({length: steps}, (_, i) => this._step(i / steps, sp, mp, fp)).map((v, i, a) => {
                ...
                return new cc.Tween().to(0.5 / steps, { position: v, angle: - sub ? (Math.atan2(sub.y, sub.x) / Math.PI * 180 - 180) : 0 })
            })
            return cc.tween()
                    .delay(0.5)
                    .to(0, { opacity: 255 })
